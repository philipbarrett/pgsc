...
The panel regression fails to recover the correct value of $b$ due to the correlation of $X_{it}$ with $D_{it}$.  Instead, it provides point estimates for $b$ close to $(0.3,0.3)$ and with neither coefficient statistically significant.
...
The function `gsc.wrapper` computes a number of variants of the GSC estimator using an iterative approach.  It works by optimizing over the coefficients $b$ and the weights $W$ each in turn, iterating until the maximum difference in iterations is suitably close to zero.[^1]
...
<!-- The first, a na\"{i}ve bootstrap, simply resamples the units.  This captures the sampling uncertainty due to the units, but fixes the ata conditional on the unit.  The obvious difficulty with bootstrapping in this manner is that it is highly likely that two (or more) identical units will be selected, yielding an identical counterfactual for those units for any treatment coefficient $b$.  The function `gsc.bootstrap` accounts for this by dropping exact duplicates and increasing their weights in the weighted objective function to reflect their multiplicity.  The output is a distribution of estimates for $b$ and $W$.  These (joint) distributions can be used to test hypotheses by checking the fraction of the bootstrapped sample which satisfies the hypothesis. -->
...
The package also includes functions for hypothesis testing, using the bootstrap method proposed in [@powell2017synthetic].  This approach requires re-estimating the model under the hypothesized restriction $g(\theta)=0$.  If the null hypothesis is true then the restriction does not bind, and the slope of the objective function should be zero when the constraint is enforced.  Bootstrapping only the final step of the calculation of the gradients of the objective function provides a quick way to approximate the distribution of the gradient, testing if the deviation from zero is statistically significant.

We start by defining the restriction(s) to be tested.  Both the function and gradient are required.

```{r}
rest.val <- 1
g <- function(b) b[1] / ( 1 - b[2] ) + rest.val ; g.grad <- function(b) c( 1 / ( 1 - b[2] ), b[1] / ( 1 - b[2] )^2 )
```

Note that the current version of the package only allows for single equation restrictions to be tested.  Extending this is a point for further development.  In the meantime, multiple restrictions can be implemented as a sum of squares of individual restrictions.  To test the restriction, we first solve the restricted model, using the auxiliary inputs `g.i` and , then test that $g(b)=0$.

```{r include=FALSE}
sol.2.step.rest <- gsc.wrapper(dta, dep.var = 'y', indep.var = c('D1','D2'), b.init = b.init,
...
With a p-value of over 0.98, the hypothesis cannot be rejected at any reasonable significance level.  This is  relief, as it is false; when evaluated at the true value of $b=(1,2)$, $g(b)=0$.  We can check that a false hypothesis is rejected by re-solving the under a false restriction:

```{r include=FALSE}
rest.val <- 2
sol.2.step.rest.2 <- gsc.wrapper(dta, dep.var = 'y', indep.var = c('D1','D2'), b.init = b.init,
                      method='twostep.indiv', g.i=g, g.i.grad=g.grad )
wald.test.g.2 <- gsc.wald.wrapper( dta, dep.var = 'y', indep.var = c('D1','D2'),
                                 sol.rest = sol.2.step.rest.2, n.boot = 10000 )
```

The resulting p-value is close to 0.02.
