...
Note that the current version of the package only allows for single equation reqtrictions to be tested.  Extending this is a point for further development.  In the meantime, multiple restrictions can be implemented as a sum of squares of individual restrictions.  To test the restriction, we first solve the restricted model, using the axuillary inputs `g.i` and , then test that $g(b)=0$.
...
With a p-value of over 0.98, the hypothesis cannot be rejected at any reasonable significance level.  This is  relief, as it is false; when evaluated at the true value of $b=(1,2)$, $g(b)=0$.  We can check that a false hypothesis is rejected byt resolveing the under a false restriction:
...
The resulting p-value is close to 0.01.
